      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        >
          ...
          <div style={{ position: "absolute", top: 18, left: 0, right: 0 }}>
            <Knob
              value={waves.indexOf(config.wave)}
              min={0}
              max={5}
              label="WAVE"
              step={1}
              onChange={(v) =>
                onChange({ ...config, wave: waves[Math.round(v)] })
              }
              size="small"
            />
          </div>
        ...
        >
          <span style={{ fontSize: 7, color: "#888" }}>SYNC TO OSC3</span>
          <Toggle
            active={config.sync}
            label=""
            color="red"
